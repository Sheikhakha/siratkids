...
- `js/quran_source/mushaf-layout-{17,18,7}.js` - QUL mushaf page-layout bundles (`__QURAN_DATA["mushaf-layout-{id}"]`): per-page line structure (run-length word locations), `ayah_page` (ayah->first page), `page_first`/`page_last`, built by `scripts/build_qul_mushaf.py`. Layouts: 17 = Indopak Hanafi compressed (13 lines, 849 pages, default), 18 = Indopak Madinah normal (17 lines, 487 pages), 7 = Indopak Nastaleeq (16 lines, 548 pages). Word text is not duplicated - the reader composes it from `indopak-nastaleeq-word.js`.
- `js/quran_source/mushaf-meta.js` - mushaf layout registry (`default_layout`, per-layout name/font/page counts)
- `js/quran_source/tajweed-rules.js` - word->tajweed rule classes map (~66k words), scraped from QUL's public `/tajweed_words/{S:V:1}` pages (full-ayah `qpc-hafs tajweed-new` preview with `<r class=RULE>` letters); rule classes mirror QUL's export CSS colors
- `css/qul-mushaf-fonts.css` - generated: 4 QUL @font-face blocks + per-layout font families + tajweed rule colors (scoped `.qr-mushaf-page .izhar` etc.) + char-end/pause colors + dark-theme relaxations
- `fonts/indopak-nastaleeq-{waqf-lazim,hanafi-compressed,madinah-normal}.woff2` + `surah-name-v4.woff2` - small (76-95 KB) QUL mushaf fonts
...
- `scripts/build_qul_mushaf.py` - Builds the mushaf layout bundles + tajweed rules + fonts + CSS (reuses `build_qul_data.py` fetch/cache/rate-limit and `write_wrapped_js`); `--dry-run` probes, `--skip-tajweed`/`--skip-fonts`/`--only-layout N` available; Madani layout id 1 deliberately dropped (Q2 decision: Indopak only, ~330 KB fonts)
...
- **Mushaf view** (page-based): the navbar Mushaf button toggles `#qr-mushaf` (page rendering from the `mushaf-layout-*` bundles, `#qr-content` hidden). `.qr-mushaf-group` in `qr-nav-right` holds the layout select + inline prev/page/next. Word spans carry `data-loc`/`data-wi` (word index within ayah), tajweed rule classes (`izhar`, `ikhafa`, `madda_*`, ...) applied from `tajweed-rules.js`, and playback highlighting reuses `getWordTimingMap` via `updateMushafWordHighlight` (scoped to the ayah being played); `startVersePlayback` auto-flips pages via `ensureMushafPageForAyah`. Clicking a word plays that ayah (single); the Go-to dialog jumps to the ayah's page; the surah sidebar jumps to the surah's first page. Layouts/bundles/tajweed load lazily (`QUL_LAZY_KEYS`).
...
- **Auth/sync model (`js/auth.js`)**: anonymous Firebase login; every page guard-redirects to `login.html` unless `sk-auth-uid` is cached. All 23 synced keys (`dark-mode`, `accent-color`, `arabic-font`, `ar-font-scale`, `en-font-scale`, `quran-wbw-scale`, 3 toggles, `audio-speed`, `audio-voice-name`, `quran-*` settings, `mushaf-tajweed`, `quran-bookmarks`, `quran-last-read`, `sk-kid-name`) + every `sk-worksheet-*` record push to Firestore `users/{uid}` (`preferences` + `worksheets`), debounced 2s via a `localStorage.setItem` patch in `main.js`. **Per-name device vault** `sk-vault-<slug>:{prefs,worksheets}` keeps kids isolated on a shared device and restores a returning kid (same name) after anonymous sign-out; `clearWorksheetIDB()` wipes `sk-worksheet-*` from the shared IndexedDB store (`siratkids/kv`) on identity switch. Firestore is the cross-device backup; the vault is the device-local source of truth. Two identical names on one device = one profile.
